  ...
  configs = merge([
    {
      ...
      "rule 100 outbound-interface"  = var.config.interfaces.wan.device
...
resource "vyos_config_block_tree" "nat-destination" {
  ...
  configs = merge(
    merge([
      for index, rule in var.config.nat.port-forwards :
      {
        ...
        "rule ${100 + index} inbound-interface" : var.config.interfaces[rule.interface].device
